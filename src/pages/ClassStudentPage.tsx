        ...
        <div className="player-main" style={{ maxWidth: 520 }}>
          <EmptyState icon="🔎" title="Klas niet gevonden">
            ...
            <div style={{ display: 'flex', gap: 8, justifyContent: 'center', flexWrap: 'wrap' }}>
              <Link to="/klas/open" className="btn btn-primary">📦 Klaspakket openen</Link>
  ...
  return (
    ...
    >
      <header className="player-topbar">
        ...
        {student && <span className="badge">👤 {student.name}</span>}
      </header>

      <div className="player-main" style={{ maxWidth: 640 }}>
        {!student ? (
          ...
          <>
            ...
            <div className="card card-pad" style={{ marginTop: 18 }}>
              <h2 style={{ fontSize: '1rem', marginTop: 0 }}>📦 Op een ander toestel werken?</h2>
              ...
              <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                <Link to="/klas/open" className="btn btn-sm btn-ghost">📦 Pakket of klaslink openen</Link>
                <Link to="/voortgang" className="btn btn-sm btn-ghost">📈 Mijn voortgang</Link>
              </div>
            </div>
...
function AssignmentList({
  ...
  if (assignments.length === 0) {
    return (
      <EmptyState icon="🌤️" title="Nog geen opdrachten">
  ...
  return (
    <section aria-label="Mijn opdrachten" style={{ display: 'grid', gap: 10, marginTop: 10 }}>
      <h2 style={{ fontSize: '1.05rem', margin: 0 }}>📋 Mijn opdrachten</h2>
      {assignments.map((a) => {
        ...
        const label =
          status.state === 'ingediend'
            ? a.kind === 'course' ? '✔ Nalezen' : '↻ Opnieuw maken'
            : status.state === 'bezig'
              ? '▶ Verder doen'
              : '▶ Starten';
        ...
        return (
          <article key={a.id} className="card card-pad" style={{ display: 'grid', gap: 8 }}>
            <div style={{ display: 'flex', gap: 8, alignItems: 'baseline', flexWrap: 'wrap' }}>
              <span aria-hidden style={{ fontSize: '1.2rem' }}>{a.kind === 'course' ? '📖' : '🧩'}</span>
...
function HandInSection({
  ...
  return (
    <section className="card card-pad" style={{ marginTop: 18 }} aria-label="Inleveren">
      <h2 style={{ fontSize: '1.05rem', marginTop: 0 }}>📮 Inleveren</h2>
      {items.length === 0 ? (
        <p className="hint" style={{ margin: 0 }}>
        ...
        <>
          ...
          {open.length === 0 ? (
            <p className="hint" role="status" style={{ margin: 0 }}>✔ Alles is doorgegeven. Netjes!</p>
          ) : (
    ...
    <div className="card" style={{ padding: '12px 14px' }}>
      ...
      {!code ? (
        <button className="btn btn-primary btn-sm" style={{ marginTop: 8 }} disabled={busy} onClick={() => { void maak(); }}>
          {busy ? 'Code wordt gemaakt…' : '📱 Toon mijn code'}
        </button>
      ) : (
        <div style={{ marginTop: 10, display: 'grid', gap: 8 }}>
          <CodeQr value={code} label={`de code van ${item.title}`} size={170} copyLabel="Code kopiëren" />
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
            <button className="btn btn-sm btn-primary" onClick={() => { void kopieer(); }}>📋 Kopiëren</button>
            <button
